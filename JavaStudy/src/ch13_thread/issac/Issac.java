...
public class Issac {
	...
	// 것을 방지하기 위해서 synchronized(동기화 유지 시켜줘야함) -- 누군가 사용중이면 건드리지 못하게 제한
	public synchronized void makeToast() {
		toast += 1;
		System.out.println("토스트를 1개 만들었습니다. 재고: " + toast);
		notify(); // 대기중(wait)인 모든 스데르를 깨운다.
	}
	...
	public synchronized void buyToast(String name, int count) {
		...
		orderList.remove(); // 처리된 주문제거(큐 구조라 처음부터 삭졔)
		notify(); // 다음 대기중인 주문 처리를 위해 모든 스레드 깨움
	}
}