...
//		System.out.println(searchSong("노래", "사랑", playList));
		...
		while(true) {
			System.out.println("=============== 멜론 차트 탑 100 ===============");
			System.out.println("가수 or 제목 어떤걸로 알려드릴까요? (종료: q)");
			System.out.print(">>>");
	...
	// output: 리스트
	// 노래 검색 메서드 생성
	// ex) ArrayList<String> searchList = searchSong("가수", "임영웅", playList);
	// System.out.println(searchList); --> 임영웅의 노래만 출력됨
		...
		for (int i = 0; i < arr.size(); i++) {
			...
			if (option.equals("가수")) {
				...
				if (temp[1].toLowerCase().contains(keyword.toLowerCase())) {
					...
					if (temp[0].toLowerCase().contains(keyword.toLowerCase())) {
						result.add(i + 1 + ". " + arr.get(i));
					}
			}
		}
