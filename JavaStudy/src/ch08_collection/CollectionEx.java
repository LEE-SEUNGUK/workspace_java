...
public class CollectionEx {
	public static void main(String[] args) {
		...
		// 1. top 100 가수 리스트를 1줄에 5명씩 출력
		// -- top 100 playListayList의 가수 이름이 중복되지 않게
		// ex) input playListayList, output playListayList
		// 배열을 전달 받아 가수의 이름으로 set에 담고
		// 중복되지 않은 가수 playListaylist를 리턴

		// 2. 가수 이름 입력시 해당 가수의 노래를 출력
		// -- 순서 정보 or 가수명을 입력받아 해당 가수의 노래 출력
		...
		ArrayList<String> singList = sortSinger(playList);
		int count = 0;

		Scanner sc = new Scanner(System.in);
		
		for (int i = 0; i < singList.size(); i++) {
			System.out.print(i + 1 + "." + singList.get(i) + "  ");
			count++;
			if (count % 5 == 0) {
				System.out.println();
			}
		...
		while (true) {
			System.out.println("\n\n=============== 멜론 차트 탑 100 ===============");
			System.out.println("어떤 가수의 노래를 알려드릴까요? (종료: q)");
			System.out.print(">>>");
			String select = sc.nextLine();
			if (select.equalsIgnoreCase("q")) { // equalsIgnoreCase 대소문자 해결
				System.out.println("탑 100 종료");
				break;
			} else {
				System.out.println();
				ArrayList<String> userList = searchSong(select, playList);
				System.out.println("검색 결과는 총 " + userList.size() + "개 입니다.\n");
	...
	public static ArrayList<String> sortSinger(ArrayList<String> playList) {
		...
		HashSet<String> sort_result = new HashSet<>(sort);

		ArrayList<String> singer = new ArrayList<>(sort_result);
	...
	public static ArrayList<String> searchSong(String keyword, ArrayList<String> playList) {
		...
		for (int i = 0; i < playList.size(); i++) {
			String[] temp = playList.get(i).split(":");
			if (temp[1].toLowerCase().contains(keyword.toLowerCase())) {
				result.add(i + 1 + ". " + playList.get(i));
			}
		}
